  ...
  const fetchAttendance = async (userId) => {
    try {
      ...
      if (res.ok) {
        ...
        setCanClockIn(!isClockedInToday && !hasPendingApprovalFromPreviousDays && !hasApprovedLeaveToday); // Can only clock in if not already clocked in today AND no pending approvals AND no approved leave today
                    ...
                    <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <h4 className="text-sm font-semibold text-yellow-800 mb-1">First cancel your today leave!</h4>
                          <p className="text-sm text-yellow-700">Then you can clock-in.</p>
                        </div>
                      </div>
                    </div>
                  ) : (
